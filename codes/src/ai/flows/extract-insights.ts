...
const ExtractInsightsOutputSchema = z.object({
  ...
  algorithmsModels: z
    .array(z.string())
    .describe('A list of algorithms or models developed or used in the research.'),
  evaluationMetrics: z
    .array(z.string())
    .describe('A list of metrics used to evaluate the research findings.'),
  keyResults: z
    .array(z.string())
    .describe('A list of the most significant findings or outcomes of the research.'),
});
export type ExtractInsightsOutput = z.infer<typeof ExtractInsightsOutputSchema>;

export async function extractInsights(
  input: ExtractInsightsInput
): Promise<ExtractInsightsOutput> {
