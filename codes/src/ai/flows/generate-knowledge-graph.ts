...
export type GenerateKnowledgeGraphInput = z.infer<typeof GenerateKnowledgeGraphInputSchema>;

const GenerateKnowledgeGraphOutputSchema = z.object({
  nodes: z.array(
    z.object({
      id: z.string().describe('Unique identifier for the node (e.g., "concept-1", "model-2").'),
      type: z.enum(['Concept', 'Model', 'Dataset', 'Method']).describe('The type of entity.'),
  ...
  edges: z.array(
    z.object({
      id: z.string().describe('Unique identifier for the edge (e.g., "edge-1").'),
      source: z.string().describe('The ID of the source node.'),
      target: z.string().describe('The ID of the target node.'),
      label: z.string().describe('A descriptive label for the relationship (e.g., "uses", "implements", "evaluates", "proposes", "applies").'),
    })
...
export async function generateKnowledgeGraph(input: GenerateKnowledgeGraphInput): Promise<GenerateKnowledgeGraphOutput> {
