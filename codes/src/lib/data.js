...
const seedPapers = [
  ...
  },
  {
    ...
    status: 'pending',
    paperText:
      'BERT stands for Bidirectional Encoder Representations from Transformers. It is a language model that is designed to pre-train deep bidirectional representations from unlabeled text...',
  },
];
...
export const db = {
  getPapers: async () =>
    Array.from(store.papers.values()).sort(
      (a, b) =>
        new Date(b.publicationDate).getTime() -
        new Date(a.publicationDate).getTime()
    ),
  getPaper: async (id) => store.papers.get(id),
  ...
  getSummary: async (id) => store.summaries.get(id),
  createInsights: async (insights) => {
    const id = (Date.now() + Math.random()).toString();
    const newInsights = { ...insights, id };
    store.insights.set(id, newInsights);
    return newInsights;
  },
  getInsights: async (id) => store.insights.get(id),
  createKnowledgeGraph: async (kg) => {
    const id = (Date.now() + Math.random()).toString();
    const newKg = { ...kg, id };
    store.knowledgeGraphs.set(id, newKg);
    return newKg;
  },
  getKnowledgeGraph: async (id) => store.knowledgeGraphs.get(id),
};
